...
const ATTRIBUTES = [
  ...
  { id: "blue", name: "藍", color: "bg-blue-500/40" },
  { id: "red", name: "紅", color: "bg-red-500/40" },
  { id: "green", name: "翠", color: "bg-green-500/40" },
  { id: "yellow", name: "黄", color: "bg-yellow-500/40" },
...
const RARITY_N = new Set([1, 11, 21, 31]);
const RARITY_R = new Set([2, 3, 4, 12, 13, 14, 22, 23, 24, 32, 33, 34]);
const getRarity = (id: number) => RARITY_N.has(id) ? "N" : RARITY_R.has(id) ? "R" : "SR";
const getIconUrl = (id: number) =>
  `https://tamamo.dev/assets/CharacterIcon/CHR_${String(id).padStart(6, "0")}_${getRarity(id)}_0.webp`;
...
export default function App() {
  ...
  useEffect(() => {
    fetch(CHARACTER_JSON_URL)
      .then(r => r.json())
      .then((data: Record<string, CharacterData>) => {
        const map: CharacterMap = {};
        Object.entries(data).forEach(([k, v]) => { map[Number(k)] = v; });
        setCharacters(map);
      })
      .catch(() => {});
  }, []);
  ...
  const allCharacterIds = useMemo(
    () => Object.keys(characters).map(Number).sort((a, b) => a - b),
    [characters]
  );
  ...
  const filteredCharacterIds = useMemo(() => {
    return allCharacterIds
      ...
      .filter(id => activeTab === "all" || characters[id]?.color === activeTab);
  }, [activeTab, invalidIds, allCharacterIds, characters]);

  const totalAvailable = allCharacterIds.length - invalidIds.size;
        ...
        <div className="max-w-7xl mx-auto space-y-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-2">
              <span className="text-xs uppercase tracking-widest text-white/40 flex items-center gap-2">
                <ShieldAlert size={12} />
                キャラクター一覽 ({usedCharacterIds.size} / {totalAvailable})
              </span>
            </div>
          ...
          <div className="h-1 bg-white/5 rounded-full overflow-hidden">
            <motion.div 
              ...
              animate={{ width: `${totalAvailable > 0 ? (usedCharacterIds.size / totalAvailable) * 100 : 0}%` }}
            />
          </div>
